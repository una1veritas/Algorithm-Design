...
const geograph::geonode & geograph::node_nearest_to(const geopoint & pt) const {
    double dist = std::numeric_limits<double>::infinity();
	uint64_t nearest_id = 0;
    for (const auto & a_pair : nodes) {
    	double d = a_pair.second.point().distance_to(pt);
    	if (d < dist) {
    		dist = d;
    		nearest_id = a_pair.second.id();
    	}
    }
    return node(nearest_id);
}
