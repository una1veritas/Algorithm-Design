...
int main(const int argc, const char * argv[]) {
	...
	const uint32_t garbages_max = 61;
	struct datafileinfo {
		uint32_t capacity;
		uint32_t size;
		uint32_t garbages[62];
	} header;

	constexpr unsigned int buffer_size = 256;
	constexpr unsigned int header_pos = 0;
	std::fstream bfile;
	bfile.open ("data.bin", ios::in | ios::out | ios::binary);
	...
	if ( bfile.read(buffer, buffer_size) ) {
		...
		bfile.clear();
		bfile.write(buffer, buffer_size);
		cout << "written." << endl;
    } else {
