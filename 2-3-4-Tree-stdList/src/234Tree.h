...
private:
	...
	struct ListNode {
		const DataType data;
		Node234 * right;
		ListNode(const DataType & d, Node234 * r) : data(d), right(r) {}
	};
...
public:
	...
	Node234(const DataType & d, Node234 * par = NULL, Node234 * lt = NULL, Node234 * rt = NULL) {
		// construct a 2-node.
		// NULL, NULL, NULL makes a root-leaf.

		data.push_back(ListNode(d, rt));
		left = lt;
	}
