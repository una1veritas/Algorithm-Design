...
struct Node234 {
	...
	struct DataNode {
		...
		DataNode(const DataType & d, Node234 * rchild) : data(d), rightchild(rchild) { }
	};
...
private:
	...
	typedef std::list<DataNode>::iterator DataNodeItr;

public:
	// empty node
	Node234() : datacount(0), leftchild(NULL) {
		datalist.clear();
	}

	// make 2 node
	Node234(DataType & d, Node234 * lchild = NULL, Node234 * rchild = NULL)
	: datacount(1), leftchild(lchild) {
		datalist.push_back(DataNode(d, rchild));
	}
	...
	bool is_full() const {
		return datacount == data_max_size;
	}
...
private:
	std::list<DataNode>::const_iterator ub_datanode(const DataType & d) const {
		// assuming one or more data d exist(s).
		std::list<DataNode>::const_iterator i = datalist.cbegin();
		for ( ; i != datalist.end(); ++i) {
			if ( d <= i->data )
				break;
		}
		return i;
	...
	// insert the given data at an appropriate position
	// in list, assuming the node is a leaf.
	std::list<DataNode>::const_iterator insert_key(const DataType & d) {
		if ( is_full() ) {
			std::cerr << "error: insert_in_data234 failure." << std::endl;
			return datalist.cend();
		}
		//std::cout << *this << ", " << k << endl;
		if (datacount == 0) { // empty root/leaf
			leftchild = NULL;
			datalist.push_back(DataNode(d, NULL));
			return datalist.cbegin();
		}
		std::list<DataNode>::const_iterator itr = ub_datanode(d);

		datalist.insert(itr, d);
		++datacount;
		//std::cout << *this << endl;
		return itr;
	}
