...
BTreeNode::BTreeNode(const DataType & k, BTreeNode * l, BTreeNode * r) : keycount(1) {
	data[0] = k;
	child[0] = l;
	child[1] = r;
...
unsigned int BTreeNode::key_index(const DataType & k) {
	unsigned int ix;
...
const DataType & BTreeNode::key_insert(const DataType & k, const unsigned int & ix, BTreeNode * left, BTreeNode * right) {
	//cout << *this << endl;
...
const DataType & BTreeNode::key_remove(const unsigned int & ix) {
	if (ix >= keycount)
		return data[ix]; // raise error.
	unsigned int i;
	const DataType & k = data[ix];
	if (ix < keycount - 1) {
		// except the right-most key and the right most child
		for(i = ix; i < keycount; ++i) {
			data[i] = data[i+1];     // key[keycount-1] = key[keycount] is redundant
...
const DataType * BTree::find(const DataType & k) const {
	BTreeNode * node = root;
	unsigned int ix;
	for(;node != NULL;) {
		ix = node->key_index(k);
...
bool BTree::insert(const DataType & k) {
...
vector<BTree::NodeIndexPair> BTree::find_and_remove_in_leaf(const DataType & k) {
	BTreeNode * node = root;
	...
	if ( found_at.node == NULL ) {
		...
		const DataType & the_key = found_at.node->data[found_at.index];
		ix = node->keycount - 1;
		found_at.node->data[found_at.index] = node->data[ix]; // substitute predecessor
		node->data[ix] = the_key;
	}
	node->key_remove(ix);
	return path;
}

bool BTree::remove(const DataType & k) {
	BTreeNode * parent;
	BTreeNode * node;
	unsigned int cix;
